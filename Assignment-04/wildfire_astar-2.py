...
#-------------------------------------------------------------------------------------------------------------------
    ...
    def bush_gen(self,density):
        ...
        for p in range(total_bushes):
            ...
            i = random.randint(rand_no_bush_i-7, rand_no_bush_i+7)                               
    ...
    def rand_fire_at_60_sec(self,updated_env, bush_list_with_state):
        ...
        xbush, ybush = 0,0
        bush_at_fire = bush_list_with_state[0]
    ...
    def neighbour_fire_at_20_sec(self, updated_env, bush_list_with_state):
        ...
        bush_list_with_state = temp_bush_list_with_state
...
class astar_planner:
    ...
    def path_generation(self, grd2, start, end):
        ...
        grd_trc = [[-1 for col in range(250)] for row in range(250)]   
        ...
        while found == False and resign == False:
            ...
            if len(heap) == 0:              #there is no path if heap is empty
                ...
                if (dist_rad<=11):                  # end-position value 6
                    found = True
                    print("fire found")
                    break
                else:     
                    for ii,item in enumerate(motion_conn): 
                        ...
                        if 0 <= x1 and x1< 249 and 0 <= 249 and y1< 249 and grd2[x1][y1] != 1:
                            cost = math.sqrt((x1-x)**2 +(y1-y)**2) + math.sqrt((x_goal-x1)**2 +(y_goal-y1)**2)
                            
                            if(dist_from_start[x][y] + cost < dist_from_start[x1][y1]): 
