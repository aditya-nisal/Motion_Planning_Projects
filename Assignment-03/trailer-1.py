...
class World:
    ...
    def world_create(self,x,y,theta,xt,yt,theta_t):
        fig = plt.figure("Animation")
        ax = fig.add_subplot(111)
        obstacle_draw = Rectangle((self.obstacle_x, self.obstacle_y),self.obstacle_width,self.obstacle_height,color= "black")
        vehicle1_draw = Rectangle((self.vehicle1_x,self.vehicle1_y),self.vehicle_width,self.vehicle_height,color='red')
        robot_draw = Rectangle((self.robot_x,self.robot_y),self.vehicle_width,self.vehicle_height,color='orange')
        trailer_draw = Rectangle((self.trailer_x,self.trailer_y),10,self.vehicle_height,color='orange')
        ...
        boundary_t = self.boundry(xt,yt,theta_t,'trailer')
        X = []
        Y = []
        for x,y in boundary:
            X.append(x)
    ...
    def neighbours(self,x,y,theta,xt,yt,theta_t):
        neighbours = []
    ...
    def valid_point(self,x,y,theta,xt,yt,theta_t):
        ...
        if x < 1 or y < self.vehicle_height or x > 200 - self.vehicle_width or y > 200 - self.vehicle_height/2.0:
            return False
        collision_check1 = self.intersection_check(boundary,self.obstacle)
        collision_check2 = self.intersection_check(boundary,self.vehicle1)
        collision_check3 = self.intersection_check(boundary_t,self.vehicle1)
        collision_check4 = self.intersection_check(boundary_t,self.obstacle)
        if (collision_check1 or collision_check2 or collision_check3 or collision_check4):
            return False
        return True

    def boundry(self,x,y,theta,vehicle_type='car'):
    ...
    def hurestic_function(self,x,y,theta,xt,yt,theta_t,vel):
        _theta = 0
        theta = (theta+360)%360
        ...
        if self.straight_available(x,y,xt,yt) and not(x > self.robot_goal[0]-5 and y > self.robot_goal[1]-5 and x <self.robot_goal[0]+5 and y <self.robot_goal[1]+5):
            ...
            _theta = 180
        if self.intersection_check([[x-15,y],[x+200*math.cos(theta*(math.pi/180))-15,y+200*math.sin(theta*(math.pi/180))],[15+x+200*math.cos(theta*(math.pi/180)),y+200*math.sin(theta*(math.pi/180))],[x+15,y]],self.obstacle):
            obstacle_penalty +=10
        if self.intersection_check([[x-15,y],[x+200*math.cos(theta*(math.pi/180))-15,y+200*math.sin(theta*(math.pi/180))],[15+x+200*math.cos(theta*(math.pi/180)),y+200*math.sin(theta*(math.pi/180))],[x+15,y]],self.obstacle):
            obstacle_penalty +=10
        if vel < 0:
        ...
        if abs(theta-theta_t)>15 and not(x > self.robot_goal[0]-5 and y > self.robot_goal[1]-5 and x <self.robot_goal[0]+5 and y <self.robot_goal[1]+5):
            turn_penalty = 5
        hurestic = distance + _theta + reverse_penalty + turn_penalty + obstacle_penalty 
        return hurestic
    ...
    def straight_available(self,x,y,xt,yt):
        ...
        if (self.intersection_check(boundary_line,self.obstacle)) or (self.intersection_check(boundary_line,self.vehicle1)) or (self.intersection_check(boundary_line_t,self.obstacle)) or (self.intersection_check(boundary_line_t,self.vehicle1)):
            return False
        return True
    ...
    def visit_checker(self,curr_node,visited):
    ...
    def A_star_algo(self):
        ...
        while len(queue) > 0:
            ...
            queue.pop(index)
            if not (self.visit_checker([round(shortest[0]),round(shortest[1]),round(shortest[2]),round(shortest[3]),round(shortest[4]),round(shortest[5])],visited)):
                ...
                if round(shortest[0]) <= self.robot_goal[0]+5 and round(shortest[0]) >= self.robot_goal[0]-5 and round(shortest[1]) <= self.robot_goal[1]+5 and round(shortest[1]) >= self.robot_goal[1]-5 and shortest[2] <= self.robot_goal[2]+15 and shortest[2] >= self.robot_goal[2]-15:
                    return path
                neighbours = self.neighbours(shortest[0],shortest[1],shortest[2],shortest[3],shortest[4],shortest[5])
                for neighbour in neighbours:
                    print(neighbour)
                    vel = neighbour[6]
                    turn = neighbour[7]
                    t_g = g_value + (0.1*self.cost(shortest[0],shortest[1],neighbour[0],neighbour[1]))
                    t_f = t_g +(0.9*self.hurestic_function(neighbour[0],neighbour[1],neighbour[2],neighbour[3],neighbour[4],neighbour[5],vel))
...
if __name__ == "__main__":
    ...
    for point in trajectory:
        plt.cla()
        diff_drive_world.world_create(point[0],point[1],point[2],point[3],point[4],point[5])
        plt.pause(0.00001)
